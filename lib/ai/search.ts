...
export async function semanticSearch(
  ...
  // We use a raw SQL approach since Supabase doesn't have native pgvector RPC out of the box
  let sql = `
    SELECT
  ...
  // Fallback: if the exec_sql RPC isn't set up yet, use a simpler query
  if (error) {
    // Use supabase-js query builder as fallback (no vector similarity)
    ...
    if (filters.type) query_builder = query_builder.eq("type", filters.type);
    if (filters.city)
    ...
    if (filters.rooms_min)
      query_builder = query_builder.gte("rooms", filters.rooms_min);
    ...
    const { data: fallbackData } = await query_builder;
